...
//      const recastedPost = {
//        ...res.data,
//        recastedBy: userDetails.nickName,
//      };

//      // Prepend to feed
//      setPosts((prev) => [recastedPost, ...prev]);
//    } catch (err: any) {
//      console.error("Error recasting post:", err);
//      alert(err.response?.data?.message || "Could not recast");
//    }
//  };
...
//                     userImg: userDetails?.userImg || userDetails?.image,
//                     userId: userDetails?.userId,
//                   },
//                 })
//               }
...
//                 width: item.media.length > 1 ? 330 : "100%",
//                 height: 300,
//               }}
//             >
//               {uri.endsWith(".mp4") ? (
...
//           }
//         >
//           <FontAwesome6 name="comment-dots" size={20} color="gray" />
...
//             name="retweet"
//             size={20}
//             color={item.recastedBy === userDetails?.nickName ? "green" : "gray"}
//           />
...
//     elevation: 3,
//   },
//   userRow: { flexDirection: "row", alignItems: "center", paddingHorizontal:10, paddingVertical: 8 },
...
//     borderRadius: 10,
//   },
//   caption: { padding: 8, paddingHorizontal: 10, fontSize: 14, color: "#333" },
//   actionsRow: {
//     flexDirection: "row",
//     justifyContent: "space-between",
//     marginTop: 10,
//   },
//   action: { flexDirection: "row", alignItems: "center", gap: 5 , paddingHorizontal: 20, paddingVertical: 5},
//   actionText: { fontSize: 10, fontWeight: "bold", color: "#555" },
//   loadingContainer: { flex: 1, justifyContent: "center", alignItems: "center", flexDirection: "row", gap: 10 },
...
const PostScreen = () => {
  ...
  const handleLike = async (postId: string) => {
    ...
    setPosts((prev) =>
      prev.map((post) => {
        if (post._id !== postId) return post;
        const hasLiked = post.likes?.includes(userDetails.userId);
        return {
          ...post,
          likes: hasLiked
            ? post.likes.filter((id: string) => id !== userDetails.userId)
            : [...(post.likes || []), userDetails.userId],
        };
      })
    );
  ...
  const handleRecast = async (postId: string) => {
    if (!userDetails?.userId) return;
  ...
  const renderItem = ({ item }: { item: any }) => (
    <View style={styles.postContainer}>
      ...
      <View style={styles.userRow}>
        <Image
          source={{ uri: userDetails?.userImg || userDetails?.image }}
          style={styles.userImg}
        />
      ...
      {/* Recast Info */}
      {item.recastedBy && (
        <Text style={styles.recastText}>Recasted by @{item.recastedBy}</Text>
      )}
      ...
      {/* Caption */}
      {item.caption && (
        <Text style={styles.caption} numberOfLines={4} ellipsizeMode="tail">
          {item.caption}
        </Text>
      )}
      ...
      <View style={styles.actionsRow}>
        ...
        >
          <AntDesign
            ...
            color={item.recastedBy === userDetails?.nickName ? "green" : "gray"}
          />
          <Text style={styles.actionText}>{item.retweets?.length || 0}</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  nickname: { fontSize: 12, color: "gray" },
  postImg: { width: "100%", height: 300, borderRadius: 10 },
  videoPlaceholder: {
    width: "100%",
    height: 300,
  ...
  caption: { paddingHorizontal: 10, fontSize: 14, color: "#333" },
  actionsRow: {
    flexDirection: "row",
    justifyContent: "space-between",
    marginTop: 10,
